...
graph TB
subgraph "配置层"
A["application.yml<br/>数据源/连接池/JPA"]
...
- [20260602_add_base_product_table.sql:1-22](file://db_changes/20260602_add_base_product_table.sql#L1-L22)
...
- [BaseProduct.java:9-38](file://src/main/java/com/superpower/modules/category/entity/BaseProduct.java#L9-L38)
- [ReqItem.java:9-58](file://src/main/java/com/superpower/modules/requirement/entity/ReqItem.java#L9-L58)
...
## 核心组件
- 数据源与连接池
  - 使用 HikariCP 作为连接池，最大连接数、连接超时、泄漏检测阈值在配置文件中设定
...
subgraph "持久化层"
CFG["application.yml<br/>数据源/连接池/JPA"]
PRAGMA["SqliteConfig.java<br/>WAL/忙等待"]
INIT["init.sql<br/>基础表DDL"]
CHG["db_changes/*.sql<br/>版本化变更"]
...
### SQLite 连接池与事务配置
- 连接池参数
  - 最大连接数、连接超时、泄漏检测阈值在配置文件中集中定义，便于运维统一管控
- JDBC URL
...
```mermaid
flowchart TD
Start(["应用启动"]) --> LoadCfg["加载 application.yml<br/>解析数据源/连接池/JPA"]
...
- 连接池并发
  - 通过最大连接数与连接超时限制资源占用，结合泄漏检测阈值及时发现异常连接
...
## 结论
本系统采用 SQLite 作为轻量级数据库，结合 HikariCP 连接池与 WAL 模式，在保证并发性能的同时维持较低运维复杂度。通过 init.sql 与 db_changes 的版本化迁移机制，实现从基础表到业务表的有序演进。建议在后续版本中完善回滚脚本与索引补全策略，持续优化查询性能与稳定性。
