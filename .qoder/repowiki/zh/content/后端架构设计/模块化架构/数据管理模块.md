...
**所做更改**   
- 新增智能标签查询支持与过滤功能章节
- 更新数据实体设计，添加 colIntelligent 字段说明
- 更新数据管理服务功能，新增智能标签过滤逻辑
- 更新控制器 API 设计，新增 intelligent 参数支持
- 更新数据模型完整性约束，添加智能标签字段约束
- 新增智能标签与现有功能协同工作说明
...
**更新** 本次更新重点增强了智能标签功能，包括智能标签查询支持、智能标签过滤功能、智能标签在数据查询中的集成，以及智能标签与现有数据管理功能的协同工作。
...
SDTO["DataEntrySummaryDTO<br/>摘要DTO"]
...
- [DataEntrySummaryDTO.java:6-123](file://src/main/java/com/superpower/modules/data/dto/DataEntrySummaryDTO.java#L6-L123)
...
## 核心组件
- 数据实体 DataEntry：定义数据清单的完整字段集合，包含业务分类、业务域、版本划分、多年财务数据、销售与成本、图片与控标点等，支持树形层级(level)与父子关系(parent_id)，并提供克隆方法用于复制非主键字段。**新增智能标签字段 colIntelligent**。
- 数据管理服务 DataEntryService：实现树构建、查询过滤、增删改、批量操作、排序与重编号、层级修复、去重、复制/移动、图片卡片链接同步等功能，并严格校验版本状态（草稿才允许修改）。**新增智能标签过滤逻辑**。
- 控制器 DataEntryController：提供 RESTful 接口，统一返回 Result 包装，负责权限校验（版本是否可编辑）、日志记录与错误处理。**新增 intelligent 参数支持**。
- 数据仓库 DataEntryRepository：基于 JPA 提供按版本、层级、父子关系、自定义筛选等查询方法，以及聚合统计与批量删除等能力。
- DTO：DataEntryDTO 作为创建/更新输入对象；DataEntrySummaryDTO 用于查询结果输出；TreeNodeDTO 用于树形展示；ExcelImportResult 用于导入结果反馈。
...
- **智能标签字段**：colIntelligent 用于标识智能化条目，TEXT 类型，默认空字符串，"1" 表示勾选智能化，空字符串表示未勾选。
...
- **智能标签过滤**
  - query 方法新增 intelligent 参数，支持智能标签查询过滤。
  - filterByIntelligent 方法实现智能标签冒泡过滤逻辑，返回自身智能化条目及其祖先节点。
- 辅助工具
...
- **智能标签查询参数**
  - intelligent：字符串参数，"1" 表示查询智能化条目，支持智能标签冒泡过滤。
- 请求参数与响应格式
...
### 数据模型完整性约束与索引设计
- 表结构与字段
  - data_entry：主键 id，version_id、parent_id、level、sort_order、is_leaf、各业务/财务/运营字段、**智能标签字段 col_intelligent**、时间戳与操作人字段。
- 索引与优化
  ...
  - **智能标签字段 col_intelligent** 若需要按智能化状态过滤，可考虑建立索引以提升查询性能。
...
- 字段长度与类型
  ...
  - **智能标签字段 col_intelligent** 使用 TEXT 类型，默认空字符串，"1" 表示勾选智能化。
...
- **智能标签过滤**：filterByIntelligent 仅保留"智能化"条目及其祖先节点，形成闭包结果集。支持智能标签冒泡显示逻辑。
...
### 智能标签查询支持
智能标签功能为数据清单新增了智能化标记能力，支持以下查询场景：

- **基础查询**：通过 intelligent 参数控制是否启用智能标签过滤
- **冒泡过滤**：当查询智能化条目时，不仅返回自身智能化的条目，还会返回包含智能化子节点的父条目
- **闭包匹配**：智能标签过滤会形成闭包结果集，确保父子层级的一致性

### 智能标签过滤功能实现
智能标签过滤通过以下步骤实现：

1. **收集智能化条目**：遍历查询结果，收集所有 colIntelligent="1" 的条目 ID
2. **构建祖先链**：对每个智能化条目，向上查找其所有祖先节点
3. **合并结果集**：将智能化条目及其祖先节点合并为最终结果集
4. **保持层级关系**：确保返回的结果集保持原有的父子层级关系

### 智能标签在数据查询中的集成
智能标签功能与现有查询系统无缝集成：

- **参数传递**：通过 intelligent 字符串参数传递过滤条件
- **逻辑融合**：智能标签过滤在现有查询逻辑基础上进行扩展
- **性能优化**：采用高效的祖先链构建算法，避免重复查询
- **兼容性**：不影响其他查询条件的正常工作

### 智能标签与现有数据管理功能的协同工作
智能标签功能与现有数据管理功能协同工作：

- **创建/更新**：DataEntryDTO 和 DataEntrySummaryDTO 已包含 colIntelligent 字段
- **排序功能**：智能标签条目参与正常的排序逻辑
- **批量操作**：智能标签在批量删除、移动等操作中得到保持
- **导入导出**：Excel 导入导出支持智能标签字段的读写
- **层级修复**：智能标签在层级修复过程中得到正确处理

**章节来源**
- [DataEntryService.java:205-240](file://src/main/java/com/superpower/modules/data/service/DataEntryService.java#L205-L240)
- [DataEntryService.java:242-269](file://src/main/java/com/superpower/modules/data/service/DataEntryService.java#L242-L269)
...
- **智能标签依赖**：智能标签功能依赖 DataEntry 实体的 colIntelligent 字段，通过字符串存储实现布尔逻辑。
...
- 索引建议
  ...
  - **智能标签索引**：对于需要频繁按智能标签过滤的场景，可在 col_intelligent 字段上建立索引。
...
- 查询优化
  ...
  - **智能标签过滤优化**：采用集合操作和祖先链构建算法，避免重复查询。
- 缓存与异步
...
## 故障排查指南
- 版本状态错误
  - 现象：修改/删除/排序等写操作报错"已发布版本不允许修改清单"。
...
- **智能标签显示异常**
  - 现象：智能标签不显示或显示不正确。
  - 处理：检查 col_intelligent 字段值是否为"1"或空字符串，确认前端渲染逻辑。
- **智能标签过滤失效**
  - 现象：查询时 intelligent 参数无效。
  - 处理：确认参数传递格式为 intelligent=1，检查后端过滤逻辑。
- 测试验证
...
## 结论
数据管理模块通过清晰的分层设计与完善的业务服务，实现了树形数据的高效查询、灵活排序与批量操作，并提供了导入导出与预览能力。**新增的智能标签功能进一步增强了数据管理能力，支持智能化条目的快速定位与层级冒泡显示。**通过严格的版本状态校验与事务控制，保障了数据一致性与安全性。建议在生产环境中配合合理的索引与缓存策略，持续优化查询与导入导出性能。
...
### 智能标签字段说明
- **字段名称**：col_intelligent
- **数据类型**：TEXT
- **默认值**：空字符串
- **取值含义**：
  - "1"：智能化条目（已勾选）
  - 空字符串：非智能化条目（未勾选）
- **存储方式**：使用字符串存储布尔逻辑，与现有字段类型保持一致
- **应用场景**：数据清单智能化标记、查询过滤、批量操作

**章节来源**
- [V1.0.12_add_col_intelligent.sql:1-4](file://db_changes/V1.0.12_add_col_intelligent.sql#L1-L4)
- [DataEntry.java:146-147](file://src/main/java/com/superpower/modules/data/entity/DataEntry.java#L146-L147)
- [DataEntryDTO.java:46](file://src/main/java/com/superpower/modules/data/dto/DataEntryDTO.java#L46)
- [DataEntrySummaryDTO.java:42](file://src/main/java/com/superpower/modules/data/dto/DataEntrySummaryDTO.java#L42)