<template>
  <div class="page">
    ...
    <el-table :data="items" border stripe size="small">
      <el-table-column prop="value" label="名称" min-width="200" />
...
async function loadData() {
  const res = await getOptions('app_role')
...
async function handleSave() {
  ...
  if (isEdit.value) {
    await updateOption(editingId.value, form.value.value)
  } else {
    await createOption('app_role', form.value.value)
  }
